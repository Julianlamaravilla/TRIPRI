...
4. **Join barrier.** When a role completes, set its status in `session-index.md` and check the other. When both are `complete`, set `Join: ready` and run `open-questions` (consolidate + flag contradictions). On platforms with script execution, `scripts/process-checker.cjs` verifies the barrier deterministically.
