...
Last updated: 2026-08-08  (**`D-05`, aclaraciones del cliente**: `CX-30` y `CX-26` cerradas · **`CX-40`, `CX-41`, `CX-42` abiertas** · `OQ-F-105`…`OQ-F-107` y `B-10`…`B-15` nuevas)
Previous: 2026-08-07  (**ampliación técnica #2**: T30, T31, T9-b, T32, T33 respondidas · `CX-31`, `CX-38`, `OQ-N-46`, `OQ-T-3`, `OQ-T-21` cerradas · **`CX-39` abierta**)  (**registro RECONCILIADO fila por fila** — las 234 filas llevan ahora marcador de estado con evidencia. Cuestionario v3 procesado (52/55); entrevista técnica completa y aprobada (23/23))
...
|---|---:|---:|---:|---:|---:|
| **Entorno técnico** `OQ-T` | 26 | 21 | 2 | 3 | **84,6 %** |
| **Negocio y Visión** `OQ-B` | 18 | 9 | 6 | 3 | **66,7 %** |
| **No funcional** `OQ-N` | 48 | 23 | 14 | 11 | **62,5 %** |
| **Contradicciones** `CX` | 42 | 26 | 1 | 15 | **63,1 %** |
| **Funcional** `OQ-F` | 107 | 34 | 27 | 46 | **44,4 %** |
| **TOTAL** | **241** | **113** | **50** | **78** | **57,3 %** |

> 🔧 **Corrección aritmética — 2026-08-08.** Las filas de TOTAL de este cuadro y del de
> `state/session-index.md` **estaban mal sumadas**: ambas declaraban **244 filas cuando los bloques
> suman 241**, porque los totales se venían incrementando a mano en vez de recalcularse desde los
> bloques. Además `CX` estaba **infracontada**: son **26 cerradas · 1 parcial · 15 abiertas**
> (verificado fila a fila), no 24/1/17. **Los porcentajes por bloque siempre fueron correctos; el
> global no.** Regla a partir de ahora: **el TOTAL se recalcula desde los bloques, nunca se ajusta
> a mano.**

> ⚠️ **Divergencia real que queda, aislada en un solo bloque — a resolver en el *join*.**
> `OQ-B` figura aquí como **9 cerradas / 6 parciales / 3 abiertas (66,7 %)** y en
> `state/session-index.md` como **13 / 4 / 1 (83,3 %)**: el efecto de `D-04` se aplicó en el índice
> y no en estas filas. **Los otros cuatro bloques coinciden exactamente.** La horquilla que produce
> en el global es **57,3 % – 58,5 %**, es decir **1,2 puntos** — no cambia ninguna decisión, pero
> **sólo una relectura fila a fila de `OQ-B` la cierra**, y eso es trabajo de la etapa de *join*.
...
## 2.bis · Preguntas al cliente abiertas por `D-05` (2026-08-08)

Seis preguntas que nacen de las aclaraciones de la última llamada. **Las tres primeras bloquean la
planificación de la v1**; las otras tres cierran ambigüedades de redacción.

| ID | Pregunta | Por qué importa | Prio |
|---|---|---|---|
| **B-10** | **"35 reales" del plan básico, ¿es semanal o mensual?** | `B-02` fijó cobro **semanal**, pero la nota no lo dice. Es la diferencia entre ~$28 y ~$6 de ingreso por empresa/mes frente a **~$43–47 de coste**: en una lectura el negocio puede cerrar y en la otra no. Ver `CX-42` | **P0** |
| **B-11** | **"El plan piloto solo va a tener pero no limitado a una ruta"** — ¿se lee como *el piloto opera una ruta, pero el sistema no debe quedar limitado a una*? | Coincide con `C-111`. Si es así, **es la mejor noticia del lote para `CX-27`**: una ruta ≈ 1 gestor y ~40 clientes (`V-09`) reduce el v1 real a una fracción de lo dimensionado. Si significa otra cosa, cambia el alcance del piloto | **P0** |
| **B-12** | Los **administradores secundarios**, ¿son el *"supervisor"* que aparece en `C-31`, `V-02` y `V-17` y que `V-04` negó? | Cerraría `CX-14` y `CX-34` de una vez. El rol fantasma lleva abierto desde el primer lote y **el modelo de roles se decide una sola vez** | P1 |
| **B-13** | Los permisos asignables, ¿**roles fijos con excepciones puntuales** o **matriz completa de permisos por recurso**? ¿Sobre qué recursos exactamente? | **El mayor motor de alcance del lote** (`CX-40`, `OQ-F-106`). Tres roles fijos son tres constantes; una matriz por recurso es un módulo entero con una matriz de prueba combinatoria — **con un desarrollador** (`CX-27`) | **P0** |
| **B-14** | 🔴 Si la fase 1 es el plan básico y el básico **no incluye WhatsApp**, **la fase 1 sale sin los dos controles antifraude** (`C-99`). ¿Se acepta? | Es la decisión de producto más grande que queda. `D-03` ya avisó de este agujero por la vía de `CX-33`; `CX-42` llega al mismo sitio por la vía del precio. **El producto cuya razón de ser es el antifraude arrancaría sin antifraude** | **P0** |
| **B-15** | *"Los socios están por debajo de los socios"* — ¿por debajo de **los administradores secundarios**? | Errata evidente en la nota. Define la jerarquía completa: principal → secundario → socio → gestor, o alguna otra | P2 |

> ⚠️ **Nota de método**: `B-10`, `B-13` y `B-14` **no deben ir en otro cuadernillo escrito**. El
> propio cliente señaló en `B-05` que percibe repetición, y las tres son decisiones, no datos:
> se resuelven **en llamada, con alguien tomando nota**.
...
| **CX-40** | 🔴 **El modelo de roles deja de ser tres roles fijos y pasa a permisos asignables por recurso** | **`C-36`** cerró `CX-6` declarando **exactamente tres roles** (Administrador, Socio, Cobrador) **con permisos ya delimitados** — tres constantes y un `if` | **`D-05` (2026-08-08)** lo rompe en dos direcciones. (1) Aparece un **cuarto nivel**: *"el Administrador puede crear **administradores secundarios** y hacer asignaciones"*. (2) 🔴 Y la cara: *"tiene la capacidad de poder **asignar permisos sobre los recursos** para los demás usuarios"* — eso **no es un rol, es un sistema de autorización por recurso**, administrado por el cliente y **distinto en cada suscripción**. **Coste real**: modelo de datos de permisos + interfaz para concederlos + comprobación en **cada** endpoint + **una matriz de prueba que deja de ser de 3 casos y pasa a ser combinatoria**. Toca decisiones cerradas: **T17** (el servicio propio pasa de autenticación a **autorización**), el aislamiento por **RLS** (que resuelve *entre* tenants, no *dentro* de uno) y **T13**, que descartó GraphQL en parte por *"el riesgo añadido de la autorización a nivel de campo"* — riesgo que ahora entra por otra puerta. **Mismo patrón que `CX-30`: el alcance crece justo después de establecer que no cabe (`CX-27`)** — con la diferencia de que **aquí la fuente sí es autoritativa**. **Decisión antes de planificar** → `B-13`: ¿roles fijos **con excepciones puntuales** (90 % del valor por 10 % del coste) o **matriz completa**? | **P0** |
| **CX-41** | **Los términos y condiciones no consiguen el objetivo que el cliente les atribuye** | **`D-05`**: *"El aplicativo debe manejar términos y condiciones de uso de la plataforma, **para evitar que nos puedan vincular con acciones delictivas**"* | **El requisito se acepta** —aceptación versionada, con registro de quién aceptó qué versión y cuándo en el libro de auditoría— y además **es necesario para LGPD**: T21 ya había listado *"base legal"* como exigencia sin cubrir. Se registra como `OQ-F-105`. **Pero el motivo declarado no se logra así**: unos términos de uso regulan la relación entre la plataforma y sus usuarios; **no determinan la responsabilidad frente a terceros ni ante una autoridad**, y **las tiendas evalúan la actividad real, no el texto legal**. El contexto lo hace concreto: `V-29` declara la actividad *"alegal"*, `CX-33` que los suscriptores **no son empresas formales**, y `mobile-platform-constraints.md` ya avisaba de que **Google Play restringe las apps de préstamos** —incluido el acceso a fotos y ubicación precisa, que este sistema usa de forma central— con **`V-49` sin resolver**. **Necesita asesoría legal en Brasil**, no una decisión de producto. Se suma a los cuatro desconocimientos legales que el cliente ya declaró (`C-93`, `C-94`, `C-95`, `C-98`) | P1 |
| **CX-42** | 🔴 **El precio del plan básico no cubre el coste por tenant — y la única lectura viable deja la fase 1 sin controles antifraude** | **`D-05`**: *"En la fase 1 = plan piloto todas las funcionalidades del **plan de 35 reales, básico**"*. **Es el primer precio de todo el Discovery** — `OQ-B-4` y `OQ-N-41` llevaban abiertas exactamente por esto | Cruzado con el coste conocido (~$430–470/mes para 10 empresas ⇒ **~$43–47 por empresa/mes**, **WhatsApp = 61 %**), a ~5,5 BRL/USD: **mensual** ⇒ ~$6 de ingreso, pierde ~$37 con WhatsApp y ~$11 sin él; **semanal** (lo que fijó `B-02`) ⇒ ~$28, pierde ~$15 con WhatsApp y **deja ~+$10 sin él**. **Una sola casilla de cuatro es viable: semanal y sin WhatsApp en el básico** — consistente con la escala reportada en `CX-30` (*"el plan siguiente tiene AI + mensajes de WhatsApp"*). 🔴 **Y de ahí sale el hallazgo mayor**: si fase 1 = plan básico y el básico no lleva WhatsApp, **la fase 1 sale sin los dos controles antifraude** (`C-99`: el QR que libera el dinero y el extracto al cliente final). `D-03` ya advirtió eso **como consecuencia de `CX-33`**; ahora se llega al mismo agujero **por un camino independiente, la segmentación de precios**. Dos causas distintas, el mismo resultado: **el producto cuya razón de ser es el antifraude arranca sin antifraude, esta vez por diseño comercial**. Decisiones: `B-10` (¿semanal o mensual?) y `B-14` (¿se acepta la fase 1 sin antifraude?) | **P0** |
| **CX-39** | **La puerta lenta de T25 bloquea el despliegue, pero no hay ambiente donde ejecutarla** | **T25** (2026-08-01) fijó **E2E + rendimiento + DAST como bloqueantes del despliegue**, y **T24** (2026-08-02) nombró las herramientas: Playwright, Maestro, k6 y OWASP ZAP | **T32** (2026-08-07): **solo producción + Docker Compose local**, sin staging. Los cuatro instrumentos se quedan con dos destinos: el portátil o producción — y **ZAP contra producción no es una opción** en un sistema con datos financieros de terceros. **Es la misma forma que `CX-31`**: una puerta que no se puede cumplir tal como está escrita. **Mitigación ya acordada en T32** (la puerta lenta corre contra la pila local completa) que resuelve E2E y DAST pero **no el rendimiento**: k6 contra un portátil no mide producción — no hay ALB, ni NAT, ni latencia de RDS en `sa-east-1`. **Decisión pendiente**: o el objetivo de `OQ-N-44` se define como medible en local (regresión algorítmica: N+1, índices ausentes, consultas que crecen con el nº de tenants), o hace falta un staging bajo demanda (~+$20/mes, evaluado y descartado en T32) | P1 |
| **CX-38** | ~~**ISO 27001 declarado por el equipo, pero el cliente no prevé que se lo pidan**~~ ✅ **CERRADA 2026-08-07 (T33)** — es justamente `V-53` lo que decidió `CX-31` a favor de *alineado, no certificado*; cierra también `OQ-N-46` | **T21** (2026-08-02, rol técnico): *"Se va a manejar ISO 27001 + LGPD"*. **V-53** (mismo día, cliente): marca **A** — *"No lo prevemos; nuestros clientes son empresas pequeñas"* | No es una contradicción fatal —el equipo puede decidir construir con ese estándar aunque nadie lo exija—, pero **cambia la respuesta correcta a `OQ-N-46`**: sin cliente que exija el certificado, **"alineado con ISO 27001" es la lectura razonable y "certificado" sería gasto sin comprador**. También descarga la presión de `CX-31` (segregación de funciones): un estándar usado como guía de diseño admite excepciones documentadas; una certificación auditada, no. **LGPD no se ve afectada** — esa es obligatoria por ley, no por exigencia de cliente | P1 |
...
|---|---|---|---|---|
| **OQ-B-1** | ¿Cuál es el nombre real del producto? El repo es `TRIPRI`, el notebook `TryPRI`, el documento lo llama "Sistema Inteligente de Administración de Préstamos con IA", y el sistema de referencia es "TryController". | Ninguna fuente fija un nombre comercial | ⬜ **ABIERTA** — `V-15`: *"aún no hemos definido"*. Sigue sin nombre tras dos cuestionarios. Q1 | 🟡 **PARCIAL — `B-01` (2026-08-07): C, *"propongan ustedes 3 opciones y elegimos"*.** El método queda acordado pero **el nombre sigue sin existir**, y es la cuarta vez que se pregunta. Bloquea ficha de tiendas, dominio, remitente de los mensajes y contrato. ⚠️ **Acción nuestra**, ya no del cliente. **P0** |
...
| **OQ-B-4** | Si es SaaS: modelo de cobro (por unidad/ruta, por gestor, por cartera, % de recaudo) y precio objetivo | Ausente por completo. **Elevada a P0 por D-01**: el cobro del software es el **único flujo de dinero real** del producto, así que su modelo define un módulo entero de la web | 🟡 **PARCIAL** — `C-04`: cobro **por ruta / unidad de cobro activa**. ⚠️ **Tres declaraciones incompatibles sobre la periodicidad**: `C-04` por unidad, `CX-30` suscripción **semanal** con planes, `V-20` vencimiento **el día 30 del mes**. **El precio objetivo nunca se dio.** Q7, Q9 | 🟡 **PARCIAL — `B-02` (2026-08-07): C, cobro semanal por planes escalonados.** El **modelo** queda fijado; **los planes y sus precios no se indicaron**. Sin importe no se puede verificar que el precio cubra los ~$43/empresa/mes de coste (`OQ-N-41`). **P0** |
| **OQ-B-5** | ¿Cuál es el volumen **actual**: nº de clientes, préstamos activos, gestores, unidades/rutas y monto de cartera? | Sólo se dice "1 o 2 gestores" para el MVP | 🟡 **PARCIAL** — `V-09`: **~10 empresas × ~5 rutas × ~40 clientes ≈ 2.000 clientes**, ~50 rutas. **Falta el monto de cartera.** Q5, Q10, todos los NFR | ✅ **CERRADA — `V-09` + `B-03` (2026-08-07).** ~2.000 clientes · ~50 rutas · **cartera < 100.000 reales**. Ticket medio ≈ **50 reales por cliente**: préstamos muy pequeños, coherente con cobranza diaria puerta a puerta. **P0** |
...
| **OQ-B-7** | Define 3–5 métricas de éxito con estado actual, meta y método de medición | La tabla de Q10 no se puede llenar con el insumo | ⬜ **ABIERTA** — `C-07`: *"por la cantidad de suscriptores"* — no es medible. `V-22` quedó **sin responder**. Q10 | ⬜ **ABIERTA — TERCER INTENTO FALLIDO.** `C-07`: *"por la cantidad de suscriptores"*. `V-22`: sin responder. `B-03`→`B-04` (2026-08-07): **X**, *"eso depende del cobrador, no tenemos una cifra"*. 🔴 **Impide rellenar `vision-document.md` §Success Metrics con contenido válido.** No volver a preguntarlo por escrito: **resolver en llamada**. **P0** |
...
| **OQ-B-9** | ¿Presupuesto y equipo real? El chat de NotebookLM plantea "1 desarrollador junior, 16 h/semana, 3–4 meses". ¿Es ese el escenario real? | Turno 4 del chat; no confirmado como decisión | 🟡 **PARCIAL** — **Equipo cerrado por `T4`**: 1 desarrollador junior (`CX-27`). **Presupuesto sin declarar** → `OQ-N-40`. Q9, T4 | 🟡 **PARCIAL — `B-05` (2026-08-07): X**, *"ya definimos el presupuesto por llamada"*. **El equipo es 1 desarrollador (`CX-27`); el presupuesto existe pero NO está en el registro escrito.** Coste conocido ~$430–470/mes (**WhatsApp = 61 %**). Sigue sin saberse si lo absorbe la empresa o se repercute. Enlaza `OQ-N-40`. **P0** |
...
| **OQ-B-11** | ¿Qué queda **explícitamente fuera** del MVP y para qué fase? | Ninguna fuente excluye nada — riesgo alto de scope creep | 🟡 **PARCIAL** — `D-03` + `V-05`: fuera de v1 el asistente IA, reportes avanzados, módulo de facturación y orden geográfico de rutas. ⚠️ **`CX-30` lo contradice**: si el plan básico incluye IA, la IA no puede estar fuera. Q16 | 🟡 **PARCIAL — `B-06` (2026-08-07) NO decide: delega.** *"Si usted puede tener todo en la primera entrega garantizando que todo queda bien sin error, si no va paso a paso"*. Devuelve al desarrollador una decisión de alcance que sólo el cliente puede tomar, justo donde `CX-27` estableció que el alcance no cabe en una persona. **`CX-30` (IA en el plan de entrada vs. IA fuera de v1) sigue abierta.** **P0** |
...
| **OQ-B-16** | ¿Qué capacidades has considerado pero decides **no comprometer** todavía (ej. scoring crediticio automático, portal para el cliente final, pagos con tarjeta)? | Q13 sin insumo | 🟡 **PARCIAL** — `V-52`: instancia separada **posible como premium, no prevista**. `V-54`: tarjetas **no entendidas, pendientes de explicación**. Scoring automático y portal del cliente final **nunca se abordaron**. Q13 | ✅ **CERRADA — `B-08` (2026-08-07).** Scoring crediticio **más adelante** · Portal del cliente final **más adelante** · Seguro de repatriación **más adelante** · **Contrato con plantilla legal y firma en el móvil: NO** · Instancia separada por cliente **más adelante**. P1 |
| **OQ-B-17** | ¿Existe un competidor/benchmark además de TryController? ¿Por qué no seguir usando TryController y ya? | Ausente | ⬜ **ABIERTA** — Nunca se respondió por qué no seguir con TryController más allá de la falta de exportación (`CX-20`) y de control antifraude. **Sin benchmark declarado.** Q6, Q7 | ✅ **CERRADA — `B-09` (2026-08-07): A.** *"Solo TryController; lo que falta es el control antifraude y poder sacar nuestros datos"*. **No se evaluó ninguna alternativa.** ⚠️ Quedó sin responder *"¿qué hace bien TryController que no se puede perder?"* — lo que hoy se usa a diario y no se nombre **corre el riesgo de no estar**. P1 |
| **OQ-B-18** | El **cobro por el uso del software** (D-01), ¿entra en el MVP o en la fase 1 se factura por fuera (transferencia/factura manual) y el módulo llega después? | Nace de D-01. Determina si hay que integrar una pasarela desde el día 1 o no | 🟡 **PARCIAL** — ⚠️ **Contradicción nueva**: `D-03` deja el **módulo de facturación fuera de v1**, pero `V-51` elige **autoservicio — la empresa se registra sola, paga y empieza**, lo que exige facturación **en v1**. Ver `OQ-F-104`. Q14, Q16 | ✅ **CERRADA — `B-07` (2026-08-07): X**, *"lo dejamos para después de que la app pase la fase de prueba"*. **El módulo de facturación NO entra en la v1**: se factura por fuera y se activa la cuenta a mano. **Resuelve la contradicción con `V-51`** (autoservicio), que exigía cobrar dentro del sistema desde el día 1. **P0** |
...
| **OQ-F-105** | **Términos y condiciones de uso dentro del aplicativo** | 🆕 2026-08-08 (**`D-05`**): *"El aplicativo debe manejar términos y condiciones de uso de la plataforma"* | ⬜ **ABIERTA — funcionalidad nueva, no estaba en ningún documento.** Requiere: texto **versionado**, pantalla de aceptación en el alta, **registro inmutable de quién aceptó qué versión y cuándo** (encaja en el libro de auditoría de T14, no en una columna editable), y **re-aceptación cuando la versión cambie**. **Doble motivo**: lo pide el cliente **y** T21 ya había listado *"base legal"* como exigencia de LGPD sin cubrir. ⚠️ El **objetivo** que el cliente le atribuye no se consigue con esto → `CX-41`. Preguntas abiertas: ¿aceptan también los **gestores** en la app, o solo el suscriptor en la web? ¿y el **prestatario**, que no usa el sistema pero cuyos datos se tratan? | P1 |
| **OQ-F-106** | **Modelo de permisos asignables por recurso** | 🆕 2026-08-08 (**`D-05`**): el administrador principal *"tiene la capacidad de poder asignar permisos sobre los recursos para los demás usuarios"* y puede **crear administradores secundarios** | ⬜ **ABIERTA — es el mayor motor de alcance del lote** (→ `CX-40`, P0). Hay que definir: **qué recursos** son asignables (¿aprobar ventas, aprobar gastos, dar llaves, cerrar caja, ver socios, crear usuarios?), **qué granularidad** (por módulo, por acción, por ruta), si los permisos se **heredan** en la jerarquía, y si existen **plantillas de rol** o se concede uno a uno. Decide entre *roles fijos con excepciones* y *matriz completa* → `B-13` | **P0** |
| **OQ-F-107** | **Alta, vinculación y desvinculación de dispositivos** | 🆕 2026-08-08 (**`D-05`**): PIN generado por la app que **muestra el modelo del dispositivo** → aprobación del administrador → **contraseña generada por el sistema**; el administrador **vincula y desvincula**; **un solo usuario por móvil** | 🟡 **PARCIAL — el flujo principal está descrito y cierra `CX-26`.** Falta el detalle operativo: **¿caduca el PIN?** (debe caducar: un código de enrolamiento sin vencimiento es una credencial permanente), **¿qué ve el administrador** además del modelo —fecha, hora, ubicación aproximada, usuario solicitante—, **¿se avisa al administrador** cuando llega una solicitud (push de T11), y **qué pasa al desvincular con las operaciones aún sin sincronizar** → sigue siendo `OQ-F-99` (**P0**), que **sube de urgencia**: desvincular deja de ser excepcional y pasa a ser acción ordinaria de administración | P1 |
...
| **OQ-T-3** | Tamaño del equipo y experiencia real por tecnología | T4 | ~~P0~~ | ✅ **CERRADA 2026-08-02 (T4)** — *(la fila estaba desactualizada; se corrige el 2026-08-07)*. **Una persona, perfil junior**: fuerte en Python + FastAPI y AWS, **React solo web, sin experiencia móvil**; sin experiencia en React Native, Expo, criptografía en dispositivo ni motores de sincronización. **Opera además producción.** Abre `CX-27` (**P0**): el alcance comprometido no cabe, y el riesgo se concentra en la mitad móvil, que es a la vez el MVP y la tecnología sin experiencia |
...
| **OQ-T-21** | Entornos y datos de prueba anonimizados | T25, **T32** | ~~P1~~ | ✅ **CERRADA 2026-08-07 (T32).** **Solo producción + Docker Compose local**, sin staging; **coste adicional $0**. Datos: **generador sintético** (día 1) **+ copia anonimizada** (capacidad futura — hoy no hay producción que copiar). Evaluados y descartados: staging bajo demanda (~+$20/mes) y permanente compartido (~+$40–50/mes, que además obliga a compartir VPC con producción). 🔴 **Abre `CX-39`**: la puerta lenta de T25 se queda sin ambiente; mitigada contra la pila local para E2E y DAST, **no para rendimiento**. 🔴 **Regla LGPD vinculante**: jamás un `pg_dump` de producción a una máquina local — una copia que conserve importes, fechas y forma de ruta es **reidentificable** con ~2.000 clientes, luego es seudonimización y **sigue siendo dato personal** (art. 13). Camino recomendado: extraer **parámetros estadísticos** y alimentar con ellos al generador |
